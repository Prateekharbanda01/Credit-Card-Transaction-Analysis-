...
select top 5 city, sum(amount) as total_amount, sum(amount) * 1.0 * 100/(select sum(cast(amount as bigint)) from credit_card_transactions)
from credit_card_transactions
group by city
...
select top 1 * , round(100*(total_spent - prev_month_spent)/prev_month_spent,2) as perc_growth 
from (
